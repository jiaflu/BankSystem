...
public class CustomerServiceImpl implements ICustomerService {
    ...
    private String REDIS_SESSION_KEY = "LUJIAFENG:";
    private String CUSTOMER = "CUSTOMER$";
    private String LOAN_PAYMENT = "DEPOSIT_PAYMENT$";
    private String CUSTOMER_TYPE = "CUSTOMER_TYPE$";
    ...
    public List<BankCustomer> getCustomers() {
        List<BankCustomer> bankCustomerList = JsonUtils.jsonToList(jedisClient.get(REDIS_SESSION_KEY + CUSTOMER), BankCustomer.class);
        if (bankCustomerList == null) {
            BankCustomerExample example = new BankCustomerExample();
            example.setOrderByClause("cust_id desc");
            bankCustomerList = bankCustomerMapper.selectByExample(example);
            for (BankCustomer bc : bankCustomerList) {
                bc.setPassword(null);
            }
            jedisClient.set(REDIS_SESSION_KEY + CUSTOMER,JsonUtils.objectToJson(bankCustomerList));
        }
